...
def parse_camera_file(file_path):
    """
    Parse our camera format.    解析我们的相机格式。
    The format is (*.txt):格式为 (*.txt):4x4 矩阵（摄像机外特性）4x4 matrix (camera extrinsic)
    space 空间
    3x3 矩阵（摄像机内部）    3x3 matrix (camera intrinsic)
    从内在矩阵中提取焦点    focal is extracted from the intrinsc matrix
    """
...
class QFunction(nn.Module):
    ...
    def forward(self, rgb_pcd, depth, proprio, pcd, camera_extrinsics, camera_intrinsics, lang_goal_emb, lang_token_embs,
        ...
        flat_imag_features = torch.cat([p.permute(0, 2, 3, 1).reshape(b, -1, feat_size) for p in rgb], 1)  # [1, 16384, 3] 
        ...
        # 瞬间 8G->20G 2G->18G    第0次 1.9->18.9G    第1次： 5.5->19.9G 4G这里的没清除 第3次7.7G->20.0G  4: 6.25G ->19.6G
        ...
        if self.cfg.neural_renderer.use_nerf_picture:
            ...
            if nerf_target_rgb is not None:
                ...
                if self.cfg.neural_renderer.field_type =='bimanual': # 什么都不需要增加
                    # rgb_0 = rgb[0] # rgb[0] sim 5 real 0
                    # depth_0 = depth[0]
                    # pcd_0 = pcd[0]

                    rendering_loss_dict, _ = self._neural_renderer(
                        rgb=rgb[0], pcd=pcd[0], depth=depth[0], # 第一个视角下的颜色 点云数据 深度
    ...
    def render(self, rgb_pcd, proprio, pcd, lang_goal_emb, lang_token_embs,
        ...
        _, ret_dict = self._neural_renderer(
                pcd=pcd[0], # real0 sim5                 # 点云数据（Point Cloud Data）的第一个元素，可能是一个3D点云表示
...
# 基于感知器（Perceiver）模型的行为克隆（Behavioral Cloning, BC）
    ...
    def update(self, step: int, replay_sample: dict, fabric: Fabric) -> dict:
        ...
        if self._layer > 0:
            right_cp = replay_sample["right_attention_coordinate_layer_%d" % (self._layer - 1)]
            left_cp = replay_sample["left_attention_coordinate_layer_%d" % (self._layer - 1)]
            right_bounds = torch.cat([right_cp - self._bounds_offset, right_cp + self._bounds_offset], dim=1)
            left_bounds = torch.cat([left_cp - self._bounds_offset, left_cp + self._bounds_offset], dim=1)
        else:
            right_bounds = bounds
            left_bounds = bounds
        ...
        # torch.nn.utils.clip_grad_norm_(self._neural_renderer.parameters(), max_norm=1.0)
        # torch.nn.utils.clip_grad_norm_(self._q.parameters(), max_norm=1.0)
        fabric.clip_gradients(self._q._neural_renderer.parameters(), self._optimizer, max_norm=1.0, error_if_nonfinite=False)
        # fabric.clip_gradients(self._neural_renderer.parameters(), self._optimizer, max_norm=1.0)
        self._optimizer.step()

        # TODO: _summaries内容不同相关的肯定要改
        torch.cuda.empty_cache() # Mani中的没有，在bimanual中出现的 加上以后在9G和23G之间跳动
        self._summaries = {
        ...
        if to_render:
        ...
        elif (step % render_freq == 0): # 只渲染voxel
            # render the voxel for visualization
            rendered_img_right = visualise_voxel(
                voxel_grid[0].cpu().detach().numpy(),    # [10, 100, 100, 100]
                None,
            ...
            import matplotlib.pyplot as plt
            rgb_src =  obs[0][0].squeeze(0).permute(1, 2, 0)  / 2 + 0.5
            fig, axs = plt.subplots(1, 3, figsize=(15, 3))   # 使用 matplotlib 创建一个包含1行7->8列子图的图形
            # src
    ...
    def act(self, step: int, observation: dict,
        ...
        # inference !!多了一个渲染损失
        # q_trans,q_rot_grip, q_ignore_collisions,vox_grid,rendering_loss_dict  = self._q(obs,
        #                     depth,
        #                     proprio,
        #                     pcd,
        #                     extrinsics, # !! append
