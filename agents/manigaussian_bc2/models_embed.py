...
class GeneralizableGSEmbedNet(nn.Module):
    ...
    def sample_in_canonical_voxel(self, xyz, voxel_feat):   # USED
        ...
        point_feature = F.grid_sample(voxel_feat, xyz_voxel_space, align_corners=True, mode='bilinear')  # [2,1,1,1,16384] [bs, 128, 1, 1, N]
        point_feature = point_feature.squeeze(2).squeeze(2).permute(0, 2, 1)  # [B, N, 128]。  [bs, N, 128]
    ...
    def forward(self, data):
        ...
        canon_xyz = self.world_to_canonical(data['xyz'])    # [1,N,3], （从数值变成比例）min:-2.28, max:1.39  # real [1,65536,3]   [2,16384,3]
        ...
        combine_index = None
        dim_size = None
        # backbone # 传入多个参数以计算潜在特征，返回的第一个值是编码后的潜在变量，第二个值被忽略（用下划线表示）
        ...
        if self.max_sh_degree > 0:
            features_rest_maps = split_network_outputs[6] # [7] # [6]
        ...
        # feature_i = torch.rand(1,307200, 4, 3) * 0.1
        data['xyz_maps'] = data['xyz'] + xyz_maps   # [B, N, 3]           [1, 65536, 3]
        data['sh_maps'] =sh_out #  torch.rand(1,307200, 3)    # sh_out                    # [B, N, 4, 3]         [1, 65536, 4, 3]        
