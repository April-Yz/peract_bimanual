...
class NeRFTaskRecorder(object):
    ...
    def take_snap(self, scene=None, obs: Observation=None):
        ...
        self.t += 1        
        self.pbar.update(1) # update progress bar 更新进度条
        ...
        # logging.info(" take_snap 1 初始化------>问题在for中%s",self._num_views)
        for i in range(self._num_views):
            self._cam_motion.step()
            # logging.info(" take_snap 1 for-1 self._cam_motion.step()结束 %s",i)
            ...
            # logging.info(" take_snap 1 for-2 scene.step() 开始前")
            # 逐步构建仿真环境
            scene.step() # step the simulation environment
            # logging.info(" take_snap 1 for-2 scene.step() 结束")
            all_views.append(
                (self._cam_motion.cam.capture_rgb() * 255.).astype(np.uint8))
            # logging.info(" take_snap 1 for-3 all_views.append()1 结束")
            all_poses.append(self._cam_motion.cam.get_matrix())
        ...
        # logging.info(" take_snap 2 for end")
        ...
        # logging.info(" take_snap 3 append完毕")
        # restore start pose
        self._cam_motion.restore_pose()
        # logging.info(" take_snap restore")
        scene.step()
        # logging.info(" take_snap 5 结束")
